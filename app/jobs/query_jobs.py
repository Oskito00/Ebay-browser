...
def check_query(query_id):
    ...
    with app.app_context():
        try:
            ...
            current_app.logger.debug(f"Checking query: {query.keywords}")
            if not query:
            ...
            for item in items:
                if item['ebay_id'] not in existing_urls:
                    try:
                        ...
                        current_app.logger.error(f"Missing field {e} in item: {item}")
                        continue
            ...
            current_app.logger.debug(f"Found {len(new_items)} new items")
            if new_items:
                db.session.add_all(new_items)
                db.session.commit()
                current_app.logger.debug("Items committed to database")
            ...
            current_app.logger.error(f"Error checking query {query_id}: {str(e)}")
            db.session.rollback()
            raise
